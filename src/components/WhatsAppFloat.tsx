...
export default function WhatsAppFloat() {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-3">
      {/* Popup card */}
      {expanded && (
        <div className="glass-card rounded-sm p-5 w-64 border border-gold-500/22 dark:border-gold-500/20 shadow-2xl">
          <div className="flex items-center justify-between mb-3">
            <span className="font-caps text-navy-900 dark:text-cream text-xs tracking-widest uppercase">Contact Us</span>
            <button type="button" onClick={() => setExpanded(false)} className="text-navy-600/45 hover:text-navy-900 dark:text-cream/40 dark:hover:text-cream">
              <X className="w-4 h-4" />
            </button>
          </div>
          <p className="font-body text-navy-700/85 dark:text-cream/60 text-sm mb-4 leading-relaxed">
            Chat with our legal team or call us directly.
          </p>
          <div className="space-y-2">
            <a
              ...
              className="flex items-center gap-3 px-4 py-3 bg-green-500/20 border border-green-500/30 rounded-sm hover:bg-green-500/30 transition-colors"
            >
